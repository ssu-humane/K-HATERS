...
def focal_binary_cross_entropy(logits, targets, num_label, gamma=2, alpha=0.25, threshold=0.5):
    l = logits.reshape(-1)
    t = targets.reshape(-1)
    p = torch.sigmoid(l) # logit 값을 sigmoid 취해줌 -> multi-label이기 때문에 소프트맥스가 아닌 sigmoid 사용
    # alpha_tensor = torch.zeros_like(targets)+alpha
    # alpha_tensor = torch.where(targets >= threshold, alpha_tensor, 1-alpha_tensor)
    
    p = torch.where(t >= threshold, p, 1-p) # 각 라벨에 대한 확률로 변환  확률  Pt
    logp = - torch.log(torch.clamp(p, 1e-4, 1-1e-4)) # 최대 최소값 지정 및 변환 후 log 씌움 -> BCE log(Pt)
    loss = logp*((1-p)**gamma)  # (1-Pt)**r * -log(Pt) == -(1-Pt)**r * log(Pt)
    loss = num_label*loss.mean()
    return loss

def focal_multi_class(inputs, targets, alpha=1, gamma=2, reduce = True):
    loss_func = nn.CrossEntropyLoss(reduction='none')
    ce_loss = loss_func(inputs, targets)
...
def masked_cross_entropy(input1,target,mask):
    ...
    for h in range(0,mask.shape[0]):
        # attention weight과 attention value에 대해 cross_entropy(mean) 계산 (유효한 토큰에 대해서만)
        cr_ent+=cross_entropy(input1[h][mask[h]],target[h][mask[h]]) 
        # cr_ent+=cross_entropy(torch.sigmoid(input1[h][mask[h]]),target[h][mask[h]]) 
