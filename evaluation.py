...
def Eval_phase(params,encoder,target_encoder,true_labels,pred_labels,logits_all_final,target_pred_labels,target_pred_label,target_true_labels,target_logits_all,target_true_label,target_labels,target_true_label_,pred_att_all,true_att_all,att_mask_all):
    ...
    for i in range(params['num_classes']):
        tmp = []
        idx, tpr, fpr = tpr_fpr(true_labels, pred_labels, target_labels, i) # 반환값 모두 list
        if i==0:
    ...
    n = 1 if params['train_att'] else 12
...
def measurement(params,comp,suff,which_files='test',model=None,test_dataloader=None,device=None):
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(test_dataloader), total=len(test_dataloader), ascii=True):
            ...
            n = 2 if params['train_target'] else 1
            num_supervised_heads = params['num_supervised_heads'] if params['train_target_att'] else params['num_supervised_heads']*2
            attention_vector=outputs[n][params['supervised_layer_pos']].detach().cpu().numpy()#[:,:,0,:].detach().cpu().numpy() # 이렇게 넣어주는 게 맞는가
            ...
            if(params['train_target']):
                ...
                for i in target_logits:
                    target_pred_labels+=((sigmoid(np.array(i))>threshold )*1).tolist() #sigmoid 값이 0.5 이상이면 1로
...
def train_model(params,device):
    embeddings=None
    file_name=set_name(params)
    file_name = file_name.replace('.pickle','')

    with open(file_name+"/test_data.pickle", 'rb') as f:
            test = pickle.load(f)
    ...
    if params['train_att']:
        ...
        comp_suff(params,True)

        measurement(params,False,True,'test',model,test_dataloader,device)
        comp_suff(params,False)
        print("explainability (iou_f1): ",max(all_iou_f1))
        print("explainability (token_f1): ",max(all_token_f1))
        print("explainability (auprc): ",max(all_auc))
    else:
        comp = []
        suff = []
        for i in range(12):
...
dict_data_folder={
    '4':{'data_file': 'Data/data_processed_4.pickle', 'class_label': 'Data/classes_four.npy'},
}
...
if __name__=='__main__': 
    ...
    my_parser.add_argument('train_target',
                           metavar='--train_target',
                           type=str,
                           help='The model to use')
    my_parser.add_argument('train_target_att',
                           metavar='--train_target_att',
                           type=str,
                           help='The model to use')
    my_parser.add_argument('train_att',
                           metavar='--train_att',
                           type=str,
                           help='The model to use')
    ...
    params['multi'] = 'multi-label'#, multi-class
    ...
    params['train_target'] = True if args.train_target=='True' else False
    if params['model_to_use']=='bert_supervised':
        params['train_target_att']= True if args.train_target_att=='True' else False
        params['train_att']= True if args.train_att=='True' else False
    else:
        params['train_target_att']=False
        params['train_att']= False
        
    if (params['train_target_att'] and params['train_att']):
        pass
    else:
